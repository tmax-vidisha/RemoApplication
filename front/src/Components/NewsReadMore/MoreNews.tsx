...
const post = [
  {
    ...
    image: (
      <img
        src={scotish}
        alt="sco"
        style={{ width: "230px", height: "120px", borderRadius: "8px" }}
      />
    ),
...
const MoreNews: React.FC<IFolderProps> = (props: IFolderProps) => {
  ...
  const erewf = (rrr: any) => {
    const countTypes =
      data?.response &&
      data?.response?.filter((movie: any) => movie.fields.Reference === rrr)
        .length;
    //@ts-ignore
    return countTypes;
  };
        ...
        <Paper className={classes.cardHeight} elevation={0} sx={{ mb: 3 }}>
          <Grid style={{ backgroundColor: "white" }}>
              <Grid
                item
                xs={10}
                style={{ borderRight: "1px solid #e6e6e6", padding: "10px" }}
              >
                <Box style={{ margin: "10px", textAlign: "left" }}>
                  <p style={{ color: "#39c8cf" }}>National</p>
                </Box>
                <Grid className={classes.imgDetails}>
                  {data?.response &&
                    data?.response
                      ?.filter(
              ...
              <Grid item xs={2}>
                <Box style={{ margin: "10px", textAlign: "left" }}>
                  <p style={{ color: "#39c8cf" }}>Related Category</p>
                </Box>
                <>
