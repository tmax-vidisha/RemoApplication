...
import {
  breadcrumbsReducer,
  foldersReducer,
} from "../../../Store copy/Reducer/foldersReducer";
...
export const MyFilesPage: React.FC<IFolderProps> = (props: IFolderProps) => {
  ...
  const documentsId =
    "b!cIcBOTQ170ygV3hcQ7aiAKUmwUfuMwpCqErIQwo2ikMF_Pl86wt6RJ301fEG4lAL";
  const trashId =
    "b!cIcBOTQ170ygV3hcQ7aiAKUmwUfuMwpCqErIQwo2ikPA6Y9HwDJ4TLznL7CJStDS";
      ...
      if (selectData[i].isChecked == true && ext !== selectData[i].name) {
        ...
        try {
          ...
          const response = await fetch(
            `${BASE_PATH}/${Site_Id}/drives/${documentsId}/items/root:/${selectData[i].name}:/content`,
            {
              method: "PUT",
              headers: {
    ...
    for (let i = 0; i < selectData.length; i++) {
      ...
      if (selectData[i].isChecked == true && ext !== selectData[i].name) {
        ...
        try {
          //@ts-ignore
          const response = await fetch(
            `${BASE_PATH}/${Site_Id}/drives/${trashId}/items/root:/${selectData[i].name}:/content`,
            {
              method: "PUT",
              headers: {
                Authorization: `Bearer ${token}`,
          ...
          <Grid>
            <FormControl variant="standard" sx={{ m: 1, minWidth: 140 }}>
              ...
              <Select
                ...
                style={{ width: "100px" }}
              >
                <MenuItem value={10} onClick={sortAscending}>
                  <span className={classes.shortBy}>Newest</span>
                </MenuItem>
                <MenuItem value={20} onClick={sortDescending}>
